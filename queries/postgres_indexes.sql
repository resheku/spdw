...
ALTER TABLE track_stats_historical ADD CONSTRAINT PK_track_stats_historical_match PRIMARY KEY (match_id);
...
-- Index for fast join from Max Speeds API: JOIN track_stats_historical ON match_id
-- (Primary key on match_id already provides an index; this supporting index covers track+match lookups)
CREATE INDEX IF NOT EXISTS track_stats_historical_track_time_idx ON track_stats_historical(track_city, match_id);
