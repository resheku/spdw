...
ALTER TABLE spdw_telemetry_stats ADD CONSTRAINT PK_spdw_telemetry_stats_match PRIMARY KEY (match_id);
...
-- Index for fast join from Max Speeds API: JOIN spdw_telemetry_stats ON match_id
-- (Primary key on match_id already provides an index; this supporting index covers track+match lookups)
CREATE INDEX IF NOT EXISTS spdw_telemetry_stats_track_time_idx ON spdw_telemetry_stats(track_city, match_id);
