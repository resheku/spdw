...
-- spdw_telemetry_stats: precomputed as-of track speed baseline per match
-- One row per match_id: cumulative track mean/stddev/n_obs using only telemetry
-- from matches at the same track_city with datetime_schedule <= this match.
-- Used by the Max Speeds API to replace the expensive per-row JOIN LATERAL.
